...
export default {
  ...
  generateContent(type, files, extraIgnoreRules = [], runcom) {
    ...
    files.forEach(path => {
      ...
      if (runcom.fileLike.indexOf(base) === -1 && isDirectory(path)) {
      ...
      if (runcom['css']) {
        content.push(
          `@import "${isDirectory(path) ? modulePath + '/' + runcom['index'] : modulePath}";`
        );
      } else {
        if (type === 'import') {
          content.push(
            `import${runcom['*'] && !mm.some(base, runcom['!*']) ? ' * as' : ''} ${moduleName} from ${quotes}${modulePath}${quotes}${terminator}`
          );
        } else {
          content.push(
